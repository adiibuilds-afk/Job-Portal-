...
const runInternFreakManual = async (bot, limit = 20, bundler) => {
    console.log(`🔄 InternFreak Manual Trigger (Limit ${limit})...`);
    ...
    try {
        ...
        console.log(`📊 InternFreak Manual Complete: ${processed} new jobs, ${skipped} skipped.`);
        return { processed, skipped, action: 'complete' };

    } catch (err) {
        console.error('❌ InternFreak Manual Failed:', err.message);
        return { processed: 0, skipped: 0 };
    }
};

module.exports = { runInternFreakManual };
